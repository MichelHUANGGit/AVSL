...
class AVSL_Graph(nn.Module):
    ...
    def _linearize(self, input:torch.Tensor) -> torch.Tensor:
    ...
    def get_graph(self, feature_map:torch.Tensor, l:int) ->tuple:
    ...
    def forward(self, x:torch.Tensor, l:int)-> tuple:
        feature_map = self.base_model(x, l)
        return self.get_graph(feature_map, l)
    
class AVSL_Similarity(nn.Module):
    '''
    AVSL Model. Can be used two ways during inference:
    - (case 1) if given one batch of images, computes all pairwise similarity in the batch. 
    - (case 2) if given two batches of images, computes all pairwise similarity between batch1 and batch2
        The output should be a dict with key 'ovr_sim' containing a matrix of shape 
        (batch1, batch2) in case 2 or (batch, batch) in case 1

    During training, the model takes one batch of images and their labels, then computes the similarities with the proxies.
    Each class has its own proxy and the proxies coordinate in the embedding space is learned.
    '''
    ...
    def __init__(
        ...
        - momemtum: the momentum at which the links are updated
    ...
    def init_parameters(self):
        ...
        for l in range(self.n_layers):
            ...
            if l >= 1:
                setattr(self, f"link_{l-1}to{l}", torch.zeros(d, d).cuda())
    ...
    def update_link(self, new_link, l):
        ...
        if self.is_link_initiated[l]:
            ...
            old_link_l.data = (1-self.momentum) * old_link_l.data + self.momentum * new_link
        else:
    ...
    def get_similarity_matrix(self, emb1, cert1, emb2, cert2, l):
        ...
        nodes = torch.abs(emb1-emb2).pow(self.p) # |(B1,1,R) - (1,B2,R)|^2 -> (B1,B2,R)
